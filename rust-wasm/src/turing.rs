...
mod tests {
    ...
    fn test_hello_world() {
        // Programa de exemplo: escreve "HI" e para
        let program = vec![
            1, 0, 72, 2, 2, // estado 1, lê '0', escreve 'H' (72), move Right, vai para estado 2
            2, 0, 73, 2, -1, // estado 2, lê '0', escreve 'I' (73), move Right, Halt
        ];
        let mut tm = TuringMachine2D::new(program, None, 0, 0, 1, 0);
        assert_eq!(tm.get_state(), 1);
        assert_eq!(tm.tape.get(0, 0), 0); // fita inicialmente vazia
        tm.run(10);
        assert_eq!(tm.get_state(), -1); // Halt
        assert_eq!(tm.tape.get(0, 0), 72); // 'H'
        assert_eq!(tm.tape.get(1, 0), 73); // 'I'
    }

    #[wasm_bindgen_test(unsupported = test)]
    fn test_wildcards() {
        // Programa de exemplo: escreve 'X' em qualquer célula, move Down, em qualquer estado, para sempre
        let program = vec![
            1, CHAR_WILDCARD, 88, 4, STATE_WILDCARD as i32,
        ];
        let mut tm = TuringMachine2D::new(program, Some(vec![0,0,0 ,0,1,1, 0,2,2, 0,3,3]), 0, 0, 1, 0);
        tm.run(100);
        for x in 0..100 {
            assert_eq!(tm.tape.get(0, x), 88); // 'X'
        }
    }
}
