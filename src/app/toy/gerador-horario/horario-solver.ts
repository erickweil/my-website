...
import { cloneArrayOperator, CrossoverOperator, GAProblem, MutationOperator } from "@/lib/genetic/problem";
...
export class RegrasHorario implements GAProblem<number[]> {
...
export function solucionarQuadroHorario(
    ...
    const {bestFitness, bestGenes, generations} = ga.run({
        ...
        progressCallback: (event) => {
            ...
            postMessage?.({ 
                type: "message", 
                value: { iter: generation, depth: fitness } 
            });
        },
    });
    const stats = {
